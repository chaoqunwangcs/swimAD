        ...
        for tracker in self.tracker_list:
            if self.data_is_zipped:
                ...
                for seq in self.seq_list:
                    ...
                    if not os.path.isfile(curr_file):
                        # 额外检查: 尝试在跟踪器根目录直接查找文件
                        alt_file = os.path.join(self.tracker_fol, tracker, seq + '.txt')
                        if os.path.isfile(alt_file):
                            print('使用替代位置的跟踪文件: ' + alt_file)
                            # 确保 data 子目录存在并复制文件
                            data_dir = os.path.join(self.tracker_fol, tracker, self.tracker_sub_fol)
                            os.makedirs(data_dir, exist_ok=True)
                            import shutil
                            shutil.copy2(alt_file, curr_file)
                            print(f'已将 {alt_file} 复制到 {curr_file}')
                            continue
                            
                        print('找不到跟踪文件: ' + curr_file)
                        raise TrackEvalException(
                            '找不到跟踪文件: ' + tracker + '/' + self.tracker_sub_fol + '/' + os.path.basename(
                                curr_file))
