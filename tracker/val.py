...
def generate_dets_embs(args: argparse.Namespace, y: Path, source: Path) -> None:
    ...
    for r in args.reid_model:
        embs_path = args.project / 'dets_n_embs' / y.stem / 'embs' / r.stem / (source.parent.name + '.txt')
        embs_path.parent.mkdir(parents=True, exist_ok=True)
        with open(str(embs_path), 'wb') as f: # Use 'wb' to clear and write bytes
            # Simpler header: just the source path and reid model, use os.linesep
            header_line = f"# Reid: {r.stem}, Source: {str(source)}{os.linesep}".encode('utf-8')
            f.write(header_line)
    # --- End: Ensure files are cleared and headers written before loop ---

    for frame_idx, r in enumerate(tqdm(results, desc=f"Processing {source.parent.name}")):
        nr_dets = len(r.boxes)
...
def trackeval(args: argparse.Namespace, seq_paths: list, save_dir: Path, MOT_results_folder: Path, gt_folder: Path, metrics: list = ["HOTA", "CLEAR", "Identity"]) -> str:
    ...
    trackeval_cmd_args = [
        ...
        "--GT_LOC_FORMAT", "{gt_folder}/{seq}/gt/gt_temp.txt", # Path structure for GT files
