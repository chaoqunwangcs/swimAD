...
class View(object):
    def __init__(self, name, p1=0, p2=0, k1=0, k2=0, k3=0, fx=0, fy=0, cx=0, cy=0, fx_ratio=1, fy_ratio=1,
                 grid_info=None):
        '''
        input: 
...
class ViewAssociation(object):
    def __init__(self, target_view, source_view):
        ...
        '''
        left_up = [MARGIN_WIDTH, MARGIN_HEIGHT]
        left_bottom = [MARGIN_WIDTH, MARGIN_HEIGHT + POOL_HEIGHT]
        right_up = [MARGIN_WIDTH + POOL_WIDTH, MARGIN_HEIGHT]
        right_bottom = [MARGIN_WIDTH + POOL_WIDTH, MARGIN_HEIGHT + POOL_HEIGHT]
        left_half = [MARGIN_WIDTH, MARGIN_HEIGHT + POOL_HEIGHT/2]
        right_half = [MARGIN_WIDTH + POOL_WIDTH, MARGIN_HEIGHT + POOL_HEIGHT/2]

        if self.source_view.name == '1':
            left_up_v, left_bottom_v, right_bottom_v, right_up_v = [180.65,  581.11], [1902.94, 1604.60], [2905.95,  623.36], [1121.88,  481.94]
            source_points = [left_up_v, left_bottom_v, right_bottom_v, right_up_v]
            # four corners, inverse clock
            target_points = [left_up, left_bottom, right_bottom, right_up]


        elif self.source_view.name == '2':
            right_half_v, right_up_v, left_up_v, left_half_v = [322.18,  610.66], [1183.97, 1779.47], [3015.56,  380.79], [2021.24,  242.55]
            source_points = [right_half_v, right_up_v, left_up_v, left_half_v]
            # four corners, inverse clock
            target_points = [right_half, right_up, left_up, left_half]
        else:
...
class Drawer(object):
    ...
    def draw_image(self, image, position=(0, 0)):  # position = (x,y)
        self.image[position[1] * self.h:(position[1] + 1) * self.h,
        position[0] * self.w:(position[0] + 1) * self.w] = image

    def draw_circle(self, center, position, color, string, radius=5, thickness=3):  # center = (x,y)
        cv2.circle(self.image, (int(center[0]) + position[0] * self.w, int(center[1]) + position[1] * self.h), radius,
                   color, thickness=thickness)
        cv2.putText(self.image, string,
                    (int(center[0]) + position[0] * self.w, int(center[1]) + position[1] * self.h - 20),
                    cv2.FONT_HERSHEY_SIMPLEX, 3, color, thickness=thickness)

    def draw_str(self, string, position, color=(255, 255, 255), thickness=3):
        cv2.putText(self.image, string, (position[0] * self.w + 5, position[1] * self.h + 70), cv2.FONT_HERSHEY_SIMPLEX,
