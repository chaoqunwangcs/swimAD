...
def find_position_between_lines(x: float, y: float, lines: List, is_vertical: bool) -> Optional[int]:
    """
    判定点在哪两条线之间
    ...
    for i in range(len(lines) - 1):
        ...
        if is_vertical:
            # 垂直线：判断点是否在两条垂直线之间
            if point_between_vertical_lines(x, y, line1, line2):
                return i
        else:
            # 水平线：判断点是否在两条水平线之间
            if point_between_horizontal_lines(x, y, line1, line2):
                return i
    
    return None

def point_between_vertical_lines(x: float, y: float, line1: List, line2: List) -> bool:
    """
    判断点是否在两条垂直线之间
    ...
    # 对于垂直线，我们需要判断点的x坐标相对于线的x坐标位置
        # 获取两条线在点的y坐标处的x坐标
    x1_at_y = get_x_at_y(line1, y)
    x2_at_y = get_x_at_y(line2, y)
...
def point_between_horizontal_lines(x: float, y: float, line1: List, line2: List) -> bool:
    """
    判断点是否在两条水平线之间
    ...
    # 获取两条线在点的x坐标处的y坐标
    y1_at_x = get_y_at_x(line1, x)
    y2_at_x = get_y_at_x(line2, x)
...
def get_x_at_y(line: List, y: float) -> Optional[float]:
    """
    获取直线在指定y坐标处的x坐标
...
def get_y_at_x(line: List, x: float) -> Optional[float]:
    """
    获取直线在指定x坐标处的y坐标
...
class GridDeterminer:
    ...
    def _determine_grid_position(self, x: float, y: float, horizontal_lines: List, vertical_lines: List) -> Optional[List[int]]:
        ...
        # 判定点在哪两条垂直线之间（决定列索引）
        col = find_position_between_lines(x, y, vertical_lines, is_vertical=True)
        if col is None:
            print(f"点({x:.2f}, {y:.2f}) 不在任何垂直分割线之间")
            return None
        
        # 判定点在哪两条水平线之间（决定行索引）
        row = find_position_between_lines(x, y, horizontal_lines, is_vertical=False)
        if row is None:
